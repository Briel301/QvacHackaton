import { z } from 'zod';
import { modelRegistryEntrySchema } from './registry.js';
/**
...
export const assessModelFitInputSchema = z.object({
    ...
    execution: modelFitExecutionSchema
        .default('sequential')
        .describe('Declared co-residency assumption for aggregation. Not a scheduling instruction: the SDK does not serialize, reserve, or enforce anything.'),
    policy: z
        .literal('interactive-v1')
        .default('interactive-v1')
        .describe('Headroom policy. `interactive-v1` withholds 20% of the memory available right now, capped at 2 GiB on desktop and 1 GiB on mobile.')
});
...
export const modelFitBudgetSchema = z.object({
    totalBytes: z
        .number()
        .describe('Total budgetable memory under the result’s basis: system RAM, or the process ceiling (allowance plus current footprint) under process-memory.'),
    usedBytes: z
        .number()
        .describe('Memory in use at sample time under the same basis (system-wide, or this process).'),
    availableBytes: z
        .number()
        .describe('Headroom before the policy applies: total − used, or the process allowance under process-memory. The reserve is a share of this.'),
...
export const assessModelFitResultSchema = z.object({
    ...
    basis: modelFitBasisSchema.describe('The evidence the budget was derived from — system RAM, the per-process ceiling on iOS, a discrete GPU’s own memory, or on Windows the GPU memory budget the OS grants this process. The two device bases also require the system-memory budget to hold.'),

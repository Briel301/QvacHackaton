 ...
 */
function disablesFlashAttention(architecture) {
 ...
 */
export function estimateLlm(input) {
    ...
    assumptions.push('weights are counted at full artifact size; llama.cpp maps them by default, so those pages are file-backed and evictable rather than anonymous RAM');
    ...
    assumptions.push('the KV cache, engine overhead and compute buffers count as resident for the model’s whole lifetime; llama.cpp allocates them when the model loads, not per operation');
    if (calibration.workingPeakBytes && calibration.workingPeakBytes.upper > 0) {
        assumptions.push('one operation is assumed in flight per model; the working peak is what a single completion was measured to add on top of the resident cost');
    }
    assumptions.push('default KV-cache types are assumed; an explicit `cache-type-k`/`cache-type-v` in `modelConfig` is not expressible in a workload and would change these numbers');
    return {
        kind: 'estimate',
        estimatorVersion: LLM_ESTIMATOR_VERSION,
        persistent,
 ...
 *   assumptions array.
 */
export function kvElementBytes(facts, hasGpu) {
    ...
    if (hasGpu) {
        return {
            ...
            assumption: 'a GPU is present, so the engine may default the KV cache to q8_0 (lower bound) or keep f16 on a CPU or OpenCL backend (upper bound)'
        };
    }
    return {
        bytes: { lower: F16_BYTES_PER_ELEMENT, upper: F16_BYTES_PER_ELEMENT },
 ...
 */
export function kvCacheBytesForWidth(facts, contextTokens, bytesPerElement) {
 ...
 *    wide on purpose.
 */
function kvCacheBytes(facts, contextTokens, elementBytes, assumptions, reasons) {
    ...
    if (facts.fullAttentionInterval && facts.fullAttentionInterval > 1) {
        ...
        assumptions.push(`${facts.architecture} keeps full attention every ${interval} blocks; the remaining blocks hold a fixed-size recurrent state instead of a KV cache, and which blocks are which is engine-owned`);
        reasons.push(`KV cache sized for ${fullBlocksLower}–${fullBlocksUpper} of ${facts.blockCount} blocks (hybrid attention)`);
        return {
    ...
    if (facts.slidingWindow) {
        ...
        assumptions.push(`${facts.architecture} uses sliding-window attention with a ${facts.slidingWindow}-token window, but the file does not say which blocks are windowed; the bound spans every block windowed to every block full`);
...
function layerClassBytes(classes, facts, contextTokens, elementBytes) {
    let total = 0;
    for (const layerClass of classes) {
        const tokens = layerClass.windowed && facts.slidingWindow
 ...
 */
function ssmStateBytes(facts, recurrentBlocks) {
    if (!facts.ssmInnerSize || !facts.ssmStateSize || recurrentBlocks <= 0)
        return 0;
    const perBlock = facts.ssmInnerSize * facts.ssmStateSize + facts.ssmInnerSize * (facts.ssmConvKernel ?? 0);
